...
static const char *TAG = "trmnl_persist";
...
esp_err_t trmnl_persist_init(void)
{
    ...
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
         ...
         * or written by a newer NVS. Both are recoverable only by erasing, and
         * losing the stored credentials just means re-running /api/setup. */
    ...
    err = nvs_open(TRMNL_NVS_NAMESPACE, NVS_READWRITE, &s_nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "nvs_open(%s): %s", TRMNL_NVS_NAMESPACE, esp_err_to_name(err));
        return err;
    }
...
bool trmnl_persist_exists(const char *key)
{
    ...
    /*
     * nvs_find_key() reports the type without reading the value, which is the
     * only way to tell "stored as 0" from "never stored" for the numeric keys.
     */
    nvs_type_t type;
...
{
    if (trmnl_persist_exists(key) && trmnl_persist_get_u32(key, 0) == value) {
        return false;
    }
    return trmnl_persist_set_u32(key, value) == ESP_OK;
}
