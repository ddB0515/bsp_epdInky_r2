...
void ha_time_format_iso8601(time_t when, char *out, size_t len)
{
    ...
    out[0] = '\0';
    if (when <= 0 || when <= PLAUSIBLE_AFTER) {
        return;
    }

    struct tm tm;
    gmtime_r(&when, &tm);
    /* Explicit "+00:00" rather than "%z": everything here is UTC, and libc's
     * %z depends on a timezone actually being set, which nothing here does. */
    strftime(out, len, "%Y-%m-%dT%H:%M:%S+00:00", &tm);
}

void ha_time_iso8601(char *out, size_t len)
{
    ha_time_format_iso8601(ha_time_is_valid() ? time(NULL) : 0, out, len);
}

bool ha_time_sync_due(void)
{
    if (!ha_time_is_valid() || s_last_sync == 0) {
        return true;
    }
...
{
    esp_sntp_config_t cfg = ESP_NETIF_SNTP_DEFAULT_CONFIG(HA_SNTP_SERVER);
