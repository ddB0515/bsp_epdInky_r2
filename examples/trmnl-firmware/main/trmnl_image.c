...
static const char *TAG = "trmnl_image";
...
/*
 * Nothing here trusts the network. An image comes off a presigned URL that this
 * firmware did not construct and cannot verify, so every size is bounded before
 * it reaches an allocator or an index.
 *
...
esp_err_t trmnl_png_render(const uint8_t *data, size_t len, epd_fb_t *fb)
{
    ...
    trmnl_png_info_t info;
    esp_err_t err = trmnl_png_probe(data, len, &info);
    if (err != ESP_OK) {
        return err;
    }
...
/*
 * Upstream's own asset pipeline for the small handful of screens the server
 * (not the device) renders before a device is claimed - "sign up with this
 * Friendly ID", "no plugin assigned" - and the only place BMP has ever turned
 * up. Just the one variant matters: an uncompressed 1-bpp BITMAPINFOHEADER
 * with a 2-entry palette, which is what every monochrome BMP writer produces.
 * Nothing here trusts the network any more than the PNG path does - every
 * offset is bounds-checked against the actual body length before use.
 */
esp_err_t trmnl_bmp_render(const uint8_t *data, size_t len, epd_fb_t *fb)
{
    ...
    /* A positive height is upstream's (and BMP's own default) bottom-up row
     * order; negative means top-down. epd_bitmap1_t already has a field for
     * exactly this, so there is nothing to flip here - just record it. */
...
esp_err_t trmnl_image_render(const uint8_t *data, size_t len, epd_fb_t *fb)
{
    ...
    if (memcmp(data, k_png_magic, sizeof(k_png_magic)) == 0) {
        return trmnl_png_render(data, len, fb);
    }
    if (data[0] == 'B' && data[1] == 'M') {
        return trmnl_bmp_render(data, len, fb);
    }

    /*
     * Formats upstream accepts that this port has never seen from the server.
     * Naming them in the log is the point: if one ever appears, the next step is
     * obvious instead of being a mystery byte dump.
     */
    if (data[0] == 0xFF && data[1] == 0xD8) {
        ESP_LOGE(TAG, "server sent JPEG; this port decodes PNG only "
                      "(the P4 has a hardware JPEG decoder if this becomes real)");
    } else if (memcmp(data, "BB", 2) == 0) {
        ESP_LOGE(TAG, "server sent a G5-compressed bitmap; this port decodes PNG only");
    } else {
