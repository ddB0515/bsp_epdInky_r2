...
// Hardware layout: VDDH[7:6], VPOS[5:4], VEE[3:2], VNEG[1:0]
#define TPS65185_UPSEQ0_VDDH_STROBE_MASK    (0x03 << 6)
#define TPS65185_UPSEQ0_VDDH_STROBE_SHIFT   6
#define TPS65185_UPSEQ0_VPOS_STROBE_MASK    (0x03 << 4)
#define TPS65185_UPSEQ0_VPOS_STROBE_SHIFT   4
#define TPS65185_UPSEQ0_VEE_STROBE_MASK     (0x03 << 2)
#define TPS65185_UPSEQ0_VEE_STROBE_SHIFT    2
#define TPS65185_UPSEQ0_VNEG_STROBE_MASK    (0x03 << 0)
#define TPS65185_UPSEQ0_VNEG_STROBE_SHIFT   0
 ...
 * 
 * Sets the STANDBY bit over I2C.  The rails are off but the device stays
 * awake and register-accessible.
 *
 * @param handle Device handle
 * @return esp_err_t ESP_OK on success
 */
...
/**
 * @brief Put TPS65185 into its lowest-power sleep state
 *
 * De-asserts the WAKEUP pin, which shuts the device down entirely — including
 * its I2C interface.  Registers are not accessible again until
 * tps65185_wakeup() re-asserts WAKEUP.
 *
 * Call after tps65185_power_down() (and normally tps65185_standby()) to reach
 * the lowest quiescent current.  Sleep can only be entered through the WAKEUP
 * pin, so this is a no-op when no wakeup_pin was configured.
 *
 * @param handle Device handle
 * @return esp_err_t ESP_OK on success
 */
 ...
 * 
 * @param handle Device handle
 * @param reg Target sequence register to write (e.g. TPS65185_REG_UPSEQ0)
 ...
 * 
 * @param handle Device handle
 * @param reg Target sequence register to write (e.g. TPS65185_REG_DWNSEQ0)
 * @param vddh_strobe VDDH strobe delay
 * @param vpos_strobe VPOS strobe delay
 * @param vee_strobe VEE strobe delay
 * @param vneg_strobe VNEG strobe delay
