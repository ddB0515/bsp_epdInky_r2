...
static const char *TAG = "ha_battery";
 ...
 */
#define HA_BATTERY_MIN_PLAUSIBLE_MV 2000
...
esp_err_t ha_battery_read(ha_battery_status_t *out)
{
    ...
    out->present = !data.battery_fail && data.voltage_mv >= HA_BATTERY_MIN_PLAUSIBLE_MV;
    out->voltage_v = data.voltage_mv / 1000.0f;
    /* soc_permille is the gauge's own gas-gauge estimate (0-1000 = 0-100.0%),
     * not something derived here from a voltage curve - reported as-is. */
    out->percent = data.soc_permille / 10.0f;
     ...
     * in" in the sense the charging binary_sensor cares about. */
