 ...
 */
typedef struct {
    const jd9168_lcd_init_cmd_t *init_cmds;         /*!< Pointer to initialization commands array. Set to NULL if using default commands.
...
} jd9168_vendor_config_t;

/**
 * @brief Create LCD panel for model JD9168
 *
 ...
 *
 */
#define JD9168_PANEL_BUS_DSI_2CH_CONFIG()                \
    {                                                    \
        .bus_id = 0,                                     \
        .num_data_lanes = 2,                             \
        .phy_clk_src = 0,                                \
 ...
 *
 */
#define JD9168_PANEL_IO_DBI_CONFIG()  \
    {                                 \
        .virtual_channel = 0,         \
        .lcd_cmd_bits = 8,            \
        .lcd_param_bits = 8,          \
 ...
 *
 * @note The pixel clock is 45 MHz, not the 50 MHz in the original vendor
 *       header, and it must be paired with a 800 Mbps lane rate. At 50 MHz /
 *       900 Mbps this panel never locks onto video: the DSI bridge stops
 *       scanning the frame buffer out and no VSYNC is produced, while the
 *       host's own test pattern keeps displaying and hides the problem.
 */
#define JD9168_1024_768_PANEL_60HZ_DPI_CONFIG(px_format) \
    {                                                    \
