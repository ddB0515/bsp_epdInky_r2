 ...
 * firmware needs - the Home Assistant WebSocket host/port and long-lived
 * access token - is collected later, by ha_portal_config_page.h, once this
 * device has joined a real network and has a DHCP address a browser can
 * actually reach. Splitting the two avoids asking for a Home Assistant token
 * over a device's own open SoftAP, and keeps this first screen to the one
 * thing that has to happen before anything else can.
 *
 * Identical in structure to
 * examples/idf_epd_ha_firmware/main/portal/ha_portal_wifi_page.h - Wi-Fi
 * provisioning doesn't change between the two firmwares - with only the
 * MQTT-specific wording in the footer text swapped out.
 *
...
"<p style=\"color:#6c757d;font-size:12px;max-width:420px;text-align:center\">Once connected, this device will show the address of a second setup page for Home Assistant.</p>\n"
"</div>\n"
"</body>\n"
"</html>\n";
